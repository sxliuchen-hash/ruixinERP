...
class SupplierService {
  ...
  async delete(id) {
    ...
    try {
      const [result] = await sequelize.query(
        `SELECT COUNT(*) AS cnt FROM contracts WHERE supplier_id = ? AND status != 'terminated'`,
        { replacements: [id], type: QueryTypes.SELECT }
      );
      if (parseInt(result.cnt, 10) > 0) {
        throw new ValidationError('该供应商存在未终止的合同，无法删除');
      }
    } catch (error) {
  ...
  async _getTransactionSummary(id) {
    ...
    try {
      const [paymentResult] = await sequelize.query(
        `SELECT COALESCE(SUM(amount), 0) AS paid_total 
         FROM payments WHERE supplier_id = ? AND type = 'expense'`,
        { replacements: [id], type: QueryTypes.SELECT }
      );
    ...
    try {
      const [invoiceResult] = await sequelize.query(
        `SELECT COALESCE(SUM(total_amount), 0) AS invoice_total 
         FROM invoices WHERE supplier_id = ? AND status != 'cancelled'`,
        { replacements: [id], type: QueryTypes.SELECT }
      );
    ...
    const payable = parseFloat((contractTotal - paidTotal).toFixed(2));
