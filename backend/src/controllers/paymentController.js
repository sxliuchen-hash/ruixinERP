 ...
 *   - 权限检查（已由 requirePermission + attachPermissionDataScope 处理）
...
async function getList(req, res, next) {
  try {
    const result = await paymentService.getList(req.query, req.dataFilter);
...
async function getDetail(req, res, next) {
  try {
    const { id } = req.params;
    const payment = await paymentService.getDetail(parseInt(id, 10), req.dataFilter);
...
async function update(req, res, next) {
  try {
    const { id } = req.params;
    const payment = await paymentService.update(parseInt(id, 10), req.body, req.dataFilter);
    res.json({
      success: true,
      message: '收付款记录更新成功',
      data: payment
...
async function remove(req, res, next) {
  try {
    const { id } = req.params;
    const result = await paymentService.delete(parseInt(id, 10), req.dataFilter);
    res.json({
      success: true,
      message: '收付款记录已删除',
      data: result
...
async function confirm(req, res, next) {
  try {
    const { id } = req.params;
    const payment = await paymentService.confirm(parseInt(id, 10), req.dataFilter);
    res.json({
      success: true,
      message: '收付款已确认',
      data: payment
...
async function getReceivable(req, res, next) {
  try {
    const result = await paymentService.getReceivable(req.query, req.dataFilter);
    res.json({ success: true, data: result });
  } catch (error) {
    next(error);
  }
...
async function getPayable(req, res, next) {
  try {
    const result = await paymentService.getPayable(req.query, req.dataFilter);
    res.json({ success: true, data: result });
  } catch (error) {
    next(error);
  }
