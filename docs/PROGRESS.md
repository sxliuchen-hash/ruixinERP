...
> **主项目二次回填与 ERP 候选（2026-07-15）**：✅ 已确认 `iptt.top`/`erp.iptt.top` 全套目标地址、四套 Client ID 命名、首次部署 active-only、300 秒建档重试和权限分页累计加载；✅ ERP 候选分支 `codex/erp-unified-auth-r7-rc1` 已形成，Windows 与 Linux Node 22 完整门禁通过，远端 CI 四个 Job 全绿，发布 Tag 为 `erp-unified-auth-r7-rc1`；⏳ M-06、M-08、M-09、M-12、M-13、M-16～M-19 仍必须在服务器部署和真实联调后完成，其中 M-09 是主项目实际部署证据，不能用 ERP Tag 代填。SSH 公钥登录、真实 Secret/active key 和首个管理员测试账号尚待人工处理。
