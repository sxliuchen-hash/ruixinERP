...
> 关联文档：开发进度见 `PROGRESS.md`；已知问题与修复状态见 `代码审查报告.md`（2026-06-13 已完成一轮安全 / 性能 / 数据一致性修复）。
...
| 鉴权 | 主项目一次性 Code SSO + ERP 独立短会话 JWT；旧密码登录仅作灰度兼容 |
...
flowchart TB
    ...
    subgraph Backend["后端 (Express, :3001)"]
        ...
        Routes["routes/* (按业务域)"]
        Auth["authenticate → requirePermission<br/>→ attachPermissionDataScope → validate<br/>→ operationLog"]
        Ctrl["controllers/*"]
        Svc["services/*"]
...
```mermaid
flowchart LR
    A["HTTP 请求"] --> B["authenticate<br/>(ERP 会话 + permissionVersion)"]
    B --> C["requirePermission(code)<br/>(权限编码授权)"]
    C --> D["attachPermissionDataScope(code)<br/>(self/team/all)"]
...
主项目是用户、角色、团队关系和角色权限配置的唯一真源。主项目签发 60 秒一次性 Code，ERP 后端兑换并签发独立短会话 JWT。ERP 会话携带 `authSource`、`permissionVersion` 和权限 grant；每项 grant 采用 `{ allowed, scope }`，其中 `scope` 为 `self`、`team` 或 `all`。`admin/process/agent` 角色判断只保留给旧密码登录灰度兼容，不再作为主 SSO 会话的授权依据。

| 中间件 | 作用 |
|--------|------|
| `authenticate` | 校验 ERP Bearer JWT，注入用户、权限 grant、`authSource` 和 `permissionVersion`；主 SSO 会话还会检查权限版本是否仍有效 |
| `requirePermission(code)` | 按 ERP 权限编码校验具体页面、接口或操作；未授权返回 403 |
| `requireErpAccess()` | `erp.app.view` 的兼容封装，不再按角色白名单判断 |
| `attachPermissionDataScope(code)` / `attachDataFilter({ permissionCode })` | 按权限 grant 注入 `req.dataFilter`，由 controller 传给 service 执行数据范围过滤 |
| `requireRole()` / `requireAdmin()` | 仅用于旧密码登录灰度兼容，目标是随 legacy 登录一起下线 |

**数据隔离规则**（权限 grant）：

- `all` → `{}`，可见该权限覆盖的全部数据；
- `self` → 按 `created_by`、`owner_id` 等业务所有者字段限定当前用户；
- `team` → 由主项目团队范围服务返回成员 ID，再生成 `IN (...)` 条件；
- 权限缺失、scope 非法或团队范围无法解析时默认拒绝匹配，不能降级为全量数据。

**前端鉴权**：`router.beforeEach` 按 `meta.permission` 校验；`/sso/initiate` 创建并绑定浏览器 RP state，`/sso/callback` 只接收格式严格的一次性 Code + 同一 state，并在网络请求前清除 URL/history；路由、侧栏和按钮统一调用权限 Store；不再接收 URL 长效 Token。`meta.watermark` 继续给薪酬类页面加水印（`components/common/Watermark.vue`）。
...
├── router/index.js              # 路由 + 守卫(鉴权/权限编码/SSO callback)
...
- `api/request.js`：`baseURL=/api/v1`，请求拦截注入 ERP 会话 JWT；401 立即清除失效会话并跳登录页，403 展示权限原因；
- `stores/user.js`：持久化 `token`、`permissions`、`authSource`、`permissionVersion`，并提供 `can`、`canAny`、`scopeOf`；
- `SystemSwitch.vue`：只打开主项目地址，不拼接或传递 ERP Token；进入 ERP 的 SSO 由主项目一次性 Code 流程负责。
...
| `run-payroll-schema-migration.js` / `sync-employees.js` / `sync-history-approvals.js` | 显式薪酬结构迁移 / 数据迁移脚本；`sync-tables.js` 仅限本地开发 |
