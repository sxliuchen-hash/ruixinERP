...
const LEGACY_STANDARD_PERMISSIONS = new Set([
...
/**
 * 仅供 authSource=legacy 的旧用户名密码登录灰度兼容。
 * main_sso 必须使用主项目下发的权限 grant，禁止调用本角色映射兜底。
 */
export function legacyCan(role, permissionCode) {
  if (!isKnownPermission(permissionCode)) return false
  if (role === 'admin') return true
  if (role !== 'process' && role !== 'agent') return false
  return LEGACY_STANDARD_PERMISSIONS.has(permissionCode)
}

export function legacyScope(role, permissionCode) {
  if (!legacyCan(role, permissionCode)) return 'none'
  return role === 'agent' ? 'self' : 'all'
}
